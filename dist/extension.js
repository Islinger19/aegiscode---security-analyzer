...
// src/client.ts
var SecurityAnalyzerClient = class {
  client;
  serverUrl;
  constructor(serverUrl) {
    ...
    this.client = axios_default.create({
      baseURL: serverUrl,
      timeout: 12e4,
      // 2 minutes - AI enhancement can be slow
      headers: {
  ...
  setServerUrl(url2) {
    this.serverUrl = url2;
    this.client.defaults.baseURL = url2;
  }
  /**
   ...
   */
  async analyzeCode(sourceCode, filename = "untitled.py", includeAiExplanation = true, severityThreshold = "low") {
    ...
    if (!sourceCode || sourceCode.trim().length === 0) {
      ...
      return {
        ...
        metadata: { total_lines: 0, analysis_time_ms: 0, analyzed_at: (/* @__PURE__ */ new Date()).toISOString() }
      };
    }
    try {
      const requestBody = {
      ...
      const response = await this.client.post("/analyze", requestBody);
      ...
      if (axios_default.isAxiosError(error)) {
        ...
        return {
          success: false,
          error_message: axiosError.code === "ECONNABORTED" ? "Analysis timed out. The LLM may be slow - try disabling AI explanations in settings." : axiosError.response?.data?.detail || axiosError.message,
          vulnerabilities: [],
          summary: { total: 0, critical: 0, high: 0, medium: 0, low: 0 },
          metadata: { total_lines: 0, analysis_time_ms: 0, analyzed_at: (/* @__PURE__ */ new Date()).toISOString() }
        };
   ...
   */
  async analyzeCodeQuick(sourceCode, filename = "untitled.py") {
    try {
      const response = await this.client.post("/analyze/quick", {
        source_code: sourceCode,
        filename
      });
      return response.data;
   ...
   */
  async analyzeMultipleFiles(files, entryPoints, includeAiExplanation = true, includeCveLookup = true) {
    ...
    try {
      ...
      if (axios_default.isAxiosError(error)) {
        ...
        return {
          success: false,
          error_message: axiosError.code === "ECONNABORTED" ? "Multi-file analysis timed out. Try with fewer files or disable AI explanations." : axiosError.response?.data?.detail || axiosError.message,
          vulnerabilities: [],
          summary: { total: 0, critical: 0, high: 0, medium: 0, low: 0 },
          metadata: { total_lines: 0, analysis_time_ms: 0, analyzed_at: (/* @__PURE__ */ new Date()).toISOString() }
        };
...
var DiagnosticsManager = class {
  ...
  vulnerabilityMap = /* @__PURE__ */ new Map();
  constructor() {
   ...
   */
  updateDiagnostics(uri, vulnerabilities) {
    const diagnostics = [];
    for (const vuln of vulnerabilities) {
      ...
      diagnostic.code = {
        ...
        target: vuln.learn_more_url ? vscode.Uri.parse(vuln.learn_more_url) : vscode.Uri.parse(`https://cwe.mitre.org/data/definitions/${vuln.cwe_id?.replace("CWE-", "")}.html`)
      };
      ...
      if (vuln.fix_suggestion) {
        diagnostic.relatedInformation = [
          new vscode.DiagnosticRelatedInformation(
            ...
            `\u{1F4A1} Fix: ${vuln.fix_suggestion}`
          )
        ];
      }
   ...
   */
  getVulnerabilityAtPosition(uri, position) {
    ...
    return vulnerabilities.find(
      (v) => v.line_number === line || v.line_number <= line && (v.end_line_number || v.line_number) >= line
    );
  }
  /**
   * Get all vulnerabilities for a document
   */
  getVulnerabilities(uri) {
...
// src/views/vulnerabilityTree.ts
var vscode2 = __toESM(require("vscode"));
var VulnerabilityTreeProvider = class {
  _onDidChangeTreeData = new vscode2.EventEmitter();
  onDidChangeTreeData = this._onDidChangeTreeData.event;
  vulnerabilitiesByFile = /* @__PURE__ */ new Map();
  constructor() {
  }
  refresh() {
    this._onDidChangeTreeData.fire();
  }
  updateVulnerabilities(uri, vulnerabilities) {
    if (vulnerabilities.length === 0) {
      this.vulnerabilitiesByFile.delete(uri.toString());
    } else {
      this.vulnerabilitiesByFile.set(uri.toString(), vulnerabilities);
    }
    this.refresh();
  }
  clear() {
    this.vulnerabilitiesByFile.clear();
    this.refresh();
  }
  getTreeItem(element) {
    return element;
  ...
  getChildren(element) {
    if (!element) {
      const items = [];
      this.vulnerabilitiesByFile.forEach((vulns, uriString) => {
        ...
        const label = `${filename} (${vulns.length} issues)`;
        const item = new VulnerabilityTreeItem(
          label,
          vscode2.TreeItemCollapsibleState.Expanded,
          "file",
          uri
        );
        item.description = this.formatCountsDescription(counts);
        item.iconPath = new vscode2.ThemeIcon("file-code");
        items.push(item);
      });
      return items;
    } else if (element.type === "file" && element.uri) {
      const vulns = this.vulnerabilitiesByFile.get(element.uri.toString()) || [];
      const items = vulns.map((vuln) => this.createVulnerabilityItem(vuln, element.uri));
      return items;
    }
    return [];
  }
  createVulnerabilityItem(vuln, fileUri) {
    const item = new VulnerabilityTreeItem(
      `Line ${vuln.line_number}: ${vuln.title}`,
      vscode2.TreeItemCollapsibleState.None,
      "vulnerability"
    );
    item.description = vuln.severity.toUpperCase();
...
*${vuln.fix_suggestion}*`
  ...
  getSeverityIcon(severity) {
    switch (severity.toLowerCase()) {
      case "critical":
        return new vscode2.ThemeIcon("error", new vscode2.ThemeColor("errorForeground"));
      case "high":
        return new vscode2.ThemeIcon("warning", new vscode2.ThemeColor("editorWarning.foreground"));
      case "medium":
        return new vscode2.ThemeIcon("info", new vscode2.ThemeColor("editorInfo.foreground"));
      case "low":
        return new vscode2.ThemeIcon("lightbulb", new vscode2.ThemeColor("editorHint.foreground"));
      default:
        return new vscode2.ThemeIcon("circle-outline");
    }
  }
  formatCountsDescription(counts) {
    const parts = [];
    if (counts.critical > 0) {
      parts.push(`\u{1F534}${counts.critical}`);
    }
    if (counts.high > 0) {
      parts.push(`\u{1F7E0}${counts.high}`);
    }
    if (counts.medium > 0) {
      parts.push(`\u{1F7E1}${counts.medium}`);
    }
    if (counts.low > 0) {
      parts.push(`\u{1F7E2}${counts.low}`);
    }
    return parts.join(" ");
  }
};
var VulnerabilityTreeItem = class extends vscode2.TreeItem {
  constructor(label, collapsibleState, type, uri) {
...
// src/views/summaryTree.ts
var vscode3 = __toESM(require("vscode"));
var SummaryTreeProvider = class {
  ...
  updateSummary(summary) {
    ...
    this.lastAnalyzedAt = /* @__PURE__ */ new Date();
    this.refresh();
  }
  clear() {
    this.summary = null;
  ...
  getChildren(element) {
    ...
    const items = [];
    const totalIcon = this.summary.total === 0 ? new vscode3.ThemeIcon("check", new vscode3.ThemeColor("testing.iconPassed")) : new vscode3.ThemeIcon("shield", new vscode3.ThemeColor("errorForeground"));
    items.push(new SummaryTreeItem(
      `Total Issues: ${this.summary.total}`,
      this.summary.total === 0 ? "No vulnerabilities found! \u{1F389}" : "Click items below for details",
      totalIcon
    ));
    if (this.summary.critical > 0) {
      items.push(new SummaryTreeItem(
        ...
        new vscode3.ThemeIcon("error", new vscode3.ThemeColor("errorForeground"))
      ));
    }
    if (this.summary.high > 0) {
      items.push(new SummaryTreeItem(
        ...
        new vscode3.ThemeIcon("warning", new vscode3.ThemeColor("editorWarning.foreground"))
      ));
    }
    if (this.summary.medium > 0) {
      items.push(new SummaryTreeItem(
        ...
        "Fix when possible",
        new vscode3.ThemeIcon("info", new vscode3.ThemeColor("editorInfo.foreground"))
      ));
    }
    if (this.summary.low > 0) {
      items.push(new SummaryTreeItem(
        ...
        new vscode3.ThemeIcon("lightbulb", new vscode3.ThemeColor("editorHint.foreground"))
      ));
    }
...
var SummaryTreeItem = class extends vscode3.TreeItem {
  constructor(label, desc, icon) {
...
// src/views/accountTree.ts
var vscode4 = __toESM(require("vscode"));
var AccountTreeProvider = class {
  ...
  constructor(authService2) {
    ...
    authService2.onAuthStateChanged(() => {
      this.stats = null;
      this.refresh();
  ...
  setHistoryService(historyService2) {
    this.historyService = historyService2;
  ...
  getChildren(element) {
    ...
    if (!this.authService.isAuthenticated) {
      const loginItem = new AccountTreeItem(
        ...
        new vscode4.ThemeIcon("sign-in", new vscode4.ThemeColor("charts.blue")),
      ...
      const profileItem = new AccountTreeItem(
        user?.name || "User",
        user?.email || "",
        new vscode4.ThemeIcon("account", new vscode4.ThemeColor("charts.green")),
        vscode4.TreeItemCollapsibleState.None
      );
      ...
      items.push(new AccountTreeItem(
        "\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500",
        "",
        new vscode4.ThemeIcon("blank"),
        vscode4.TreeItemCollapsibleState.None
      ));
      if (this.stats) {
        items.push(new AccountTreeItem(
          `\u{1F4C1} Files Analyzed: ${this.stats.total_files_analyzed}`,
          "Total unique files",
          new vscode4.ThemeIcon("file-code"),
          vscode4.TreeItemCollapsibleState.None
        ));
        items.push(new AccountTreeItem(
          `\u{1F50D} Total Analyses: ${this.stats.total_analyses}`,
          "Including re-analyses",
          new vscode4.ThemeIcon("search"),
          vscode4.TreeItemCollapsibleState.None
        ));
        items.push(new AccountTreeItem(
          `\u{1F6E1}\uFE0F Vulnerabilities: ${this.stats.total_vulnerabilities_found}`,
          "Total found across all files",
          new vscode4.ThemeIcon("shield"),
          vscode4.TreeItemCollapsibleState.None
        ));
        items.push(new AccountTreeItem(
          `\u2705 Fixes Applied: ${this.stats.total_fixes_applied}`,
          "Security improvements made",
          new vscode4.ThemeIcon("check", new vscode4.ThemeColor("testing.iconPassed")),
          vscode4.TreeItemCollapsibleState.None
        ));
        if (this.stats.vulnerabilities_by_severity) {
          ...
          const severityItem = new AccountTreeItem(
            ...
            new vscode4.ThemeIcon("pie-chart"),
          ...
          severityItem.children = [
            new AccountTreeItem(
              ...
              "",
              new vscode4.ThemeIcon("error", new vscode4.ThemeColor("errorForeground")),
              vscode4.TreeItemCollapsibleState.None
            ),
            new AccountTreeItem(
              `High: ${sev.high || 0}`,
              "",
              new vscode4.ThemeIcon("warning", new vscode4.ThemeColor("editorWarning.foreground")),
              vscode4.TreeItemCollapsibleState.None
            ),
            new AccountTreeItem(
              `Medium: ${sev.medium || 0}`,
              "",
              new vscode4.ThemeIcon("info", new vscode4.ThemeColor("editorInfo.foreground")),
              vscode4.TreeItemCollapsibleState.None
            ),
            new AccountTreeItem(
              `Low: ${sev.low || 0}`,
              "",
              new vscode4.ThemeIcon("lightbulb"),
              vscode4.TreeItemCollapsibleState.None
            )
          ];
        ...
        items.push(new AccountTreeItem(
          "Loading statistics...",
          "",
          new vscode4.ThemeIcon("loading~spin"),
          vscode4.TreeItemCollapsibleState.None
        ));
      } else {
        items.push(new AccountTreeItem(
      ...
      const viewDashboardItem = new AccountTreeItem(
        "\u{1F310} Open Web Dashboard",
        "View detailed analytics online",
        new vscode4.ThemeIcon("link-external"),
        vscode4.TreeItemCollapsibleState.None
      );
      viewDashboardItem.command = {
        command: "vscode.open",
        title: "Open Dashboard",
        arguments: [vscode4.Uri.parse("http://localhost:5173/dashboard")]
      };
      items.push(viewDashboardItem);
      const refreshItem = new AccountTreeItem(
        "\u{1F504} Refresh Stats",
      ...
      const logoutItem = new AccountTreeItem(
        ...
        new vscode4.ThemeIcon("sign-out", new vscode4.ThemeColor("errorForeground")),
        vscode4.TreeItemCollapsibleState.None
      );
      logoutItem.command = {
...
var AccountTreeItem = class extends vscode4.TreeItem {
  constructor(label, description, iconPath, collapsibleState) {
    super(label, collapsibleState);
    this.label = label;
...
var HoverProvider = class {
  constructor(diagnosticsManager2) {
    this.diagnosticsManager = diagnosticsManager2;
  }
  provideHover(document2, position, _token) {
    ...
    const markdown = new vscode5.MarkdownString();
...
`);
    ...
    return new vscode5.Hover(markdown);
  }
  getSeverityEmoji(severity) {
    switch (severity.toLowerCase()) {
      case "critical":
...
var SecurityInlineCompletionProvider = class {
  diagnosticsManager;
  fixCache = /* @__PURE__ */ new Map();
  constructor(diagnosticsManager2) {
    this.diagnosticsManager = diagnosticsManager2;
  }
  /**
   ...
   */
  updateFixes(uri, vulnerabilities) {
    const fixes = [];
    for (const vuln of vulnerabilities) {
  ...
  provideInlineCompletionItems(document2, position, context, token) {
    const fixes = this.fixCache.get(document2.uri.toString());
    ...
    const currentLine = document2.lineAt(position.line);
    ...
    const item = new vscode6.InlineCompletionItem(processedFix);
    ...
    if (endLine0Based > position.line) {
      item.command = {
        ...
        arguments: [document2.uri.toString(), position.line, endLine0Based, fix.description]
      };
    } else {
      item.command = {
   ...
   */
  processFixedCode(fixedCode, documentText, indent) {
  ...
  importExistsInDocument(importLine, documentText) {
...
var SecurityCodeActionProvider = class {
  ...
  static providedCodeActionKinds = [
    ...
    vscode6.CodeActionKind.Refactor
  ];
  provideCodeActions(document2, range, context, token) {
    const actions = [];
    const vulnerabilities = this.diagnosticsManager.getVulnerabilities(document2.uri);
    const documentText = document2.getText();
    const line1Based = range.start.line + 1;
    const relevantVulns = vulnerabilities.filter(
      (v) => v.line_number === line1Based || v.line_number <= line1Based && (v.end_line_number || v.line_number) >= line1Based
    );
    for (const vuln of relevantVulns) {
      ...
      const fixAction = new vscode6.CodeAction(
        `\u{1F512} Fix: ${vuln.title}`,
        vscode6.CodeActionKind.QuickFix
      );
      const edit = new vscode6.WorkspaceEdit();
      const startLine = vuln.line_number - 1;
      const endLine = (vuln.end_line_number || vuln.line_number) - 1;
      const startLineObj = document2.lineAt(startLine);
      const endLineObj = document2.lineAt(endLine);
      const indent = startLineObj.text.match(/^(\s*)/)?.[1] || "";
      const processedFix = this.processFixedCode(vuln.fixed_code, documentText, indent);
      const replaceRange = new vscode6.Range(
        startLine,
        0,
        endLine,
        endLineObj.text.length
      );
      edit.replace(document2.uri, replaceRange, processedFix);
      fixAction.edit = edit;
      fixAction.isPreferred = true;
      fixAction.diagnostics = context.diagnostics.filter(
        (d) => d.range.start.line >= startLine && d.range.start.line <= endLine
      );
      actions.push(fixAction);
      const learnAction = new vscode6.CodeAction(
        `\u{1F4DA} Learn: ${vuln.vulnerability_type_display}`,
        vscode6.CodeActionKind.Empty
      );
      const learnUrl = vuln.learn_more_url || `https://owasp.org/www-community/vulnerabilities/${vuln.vulnerability_type}`;
      learnAction.command = {
        command: "vscode.open",
        title: "Learn More",
        arguments: [vscode6.Uri.parse(learnUrl)]
      };
...
var AuthService = class _AuthService {
  ...
  _onAuthStateChanged = new vscode7.EventEmitter();
  ...
  static getInstance(context) {
    if (!_AuthService.instance) {
      ...
      _AuthService.instance = new _AuthService(context);
    }
  ...
  async loadAuthState() {
    try {
      ...
      const userJson = this.context.globalState.get("aegiscode.user");
      if (token && userJson) {
   ...
   */
  async saveAuthState(token, user) {
  ...
  async clearAuthState() {
    ...
    await this.context.globalState.update("aegiscode.user", void 0);
    this._authState = {
      isAuthenticated: false,
      user: null,
      token: null
...
Click to manage account`;
  ...
  async verifyToken(token) {
    try {
      const config = vscode7.workspace.getConfiguration("pythonSecurityAnalyzer");
  ...
  async login() {
    ...
    const serverUrl = config.get("serverUrl", "http://127.0.0.1:8000");
    try {
      const healthCheck = await axios_default.get(`${serverUrl}/health`, { timeout: 5e3 });
    ...
    vscode7.window.showInformationMessage(
      ...
      if (selection === "Open Browser") {
        vscode7.env.openExternal(vscode7.Uri.parse(loginUrl));
      }
    });
    await vscode7.env.openExternal(vscode7.Uri.parse(loginUrl));
  }
  /**
  ...
  async handleAuthCallback(uri) {
    ...
    if (error) {
      vscode7.window.showErrorMessage(`Authentication failed: ${error}`);
      return;
    }
    if (!token) {
      vscode7.window.showErrorMessage("No authentication token received");
      return;
    }
    try {
      const config = vscode7.workspace.getConfiguration("pythonSecurityAnalyzer");
      const serverUrl = config.get("serverUrl", "http://127.0.0.1:8000");
      const response = await axios_default.get(`${serverUrl}/auth/me`, {
        headers: { Authorization: `Bearer ${token}` },
        timeout: 5e3
      });
      ...
      console.error("Failed to complete authentication:", error2);
      vscode7.window.showErrorMessage("Failed to complete authentication");
    }
  }
  /**
   * Logout the user
  ...
  async logout() {
    ...
    if (confirm === "Logout") {
      ...
      await this.clearAuthState();
      vscode7.window.showInformationMessage("You have been logged out");
    }
  }
  /**
    ...
    if (this._authState.isAuthenticated && this._authState.user) {
      ...
      if (selected) {
        if (selected.label.includes("Logout")) {
          ...
          vscode7.commands.executeCommand("pythonSecurityAnalyzer.showStats");
        } else if (selected.label.includes("History")) {
          vscode7.commands.executeCommand("pythonSecurityAnalyzer.showHistory");
        }
      }
    } else {
      const items = [
...
var HistoryService = class _HistoryService {
  ...
  static getInstance(authService2) {
    if (!_HistoryService.instance) {
      if (!authService2) {
        throw new Error("HistoryService must be initialized with authService first");
      }
      _HistoryService.instance = new _HistoryService(authService2);
    }
    return _HistoryService.instance;
  }
  updateServerUrl() {
    const config = vscode8.workspace.getConfiguration("pythonSecurityAnalyzer");
  ...
  canSaveHistory() {
    const config = vscode8.workspace.getConfiguration("pythonSecurityAnalyzer");
    const saveHistoryEnabled = config.get("saveAnalysisHistory", true);
   ...
   */
  async saveAnalysis(result, filename, filePath, sourceCode) {
    ...
    try {
      ...
      const payload = {
        ...
        source_code: sourceCode || "",
        // Always include for web app vulnerability display
        ...
        ai_explanation_enabled: config.get("enableAiExplanations", true),
        severity_threshold: config.get("minSeverity", "low"),
        source: "vscode_extension",
        extension_version: vscode8.extensions.getExtension("aegiscode---security-analyzer")?.packageJSON.version
      };
      await axios_default.post(
   ...
   */
  async recordFix(filename, filePath, vulnerability, originalCode, fixedCode) {
    if (!this.canSaveHistory()) {
      return false;
    }
    try {
   ...
   */
  async getFiles(skip = 0, limit = 50, search) {
    if (!this.authService.isAuthenticated) {
      return null;
    }
    try {
      ...
      const response = await axios_default.get(`${this.serverUrl}/history/files`, {
        params,
        headers: this.getAuthHeaders(),
        timeout: 1e4
      });
  ...
  async getFileDetail(fileId) {
    ...
    try {
      const response = await axios_default.get(`${this.serverUrl}/history/files/${fileId}`, {
        headers: this.getAuthHeaders(),
        timeout: 1e4
      });
      return response.data;
  ...
  async getDashboard() {
    ...
    try {
      const response = await axios_default.get(`${this.serverUrl}/history/dashboard`, {
        headers: this.getAuthHeaders(),
        timeout: 1e4
      });
      return response.data;
  ...
  async getStats() {
    ...
    try {
      const response = await axios_default.get(`${this.serverUrl}/history/stats`, {
        headers: this.getAuthHeaders(),
        timeout: 1e4
      });
      return response.data;
   ...
   */
  async getRecentVulnerabilities(limit = 50) {
    if (!this.authService.isAuthenticated) {
      return null;
    }
    try {
      const response = await axios_default.get(`${this.serverUrl}/history/recent-vulnerabilities`, {
        params: { limit },
        headers: this.getAuthHeaders(),
        timeout: 1e4
      });
  ...
  async showStatsPanel() {
    ...
    if (!stats) {
      vscode8.window.showErrorMessage("Failed to load statistics");
      return;
    }
  ...
  getStatsHtml(stats) {
    const topTypes = Object.entries(stats.top_vulnerability_types).slice(0, 5).map(([type, count]) => `<li>${type.replace(/_/g, " ")}: ${count}</li>`).join("");
    return `
<!DOCTYPE html>
<html>
<head>
...
<body>
    <h1>\u{1F6E1}\uFE0F Your Security Statistics</h1>
...
var CACHE_KEY = "aegiscode.analysisCache";
...
var CacheService = class _CacheService {
  ...
  static getInstance(context) {
    if (!_CacheService.instance) {
      ...
      _CacheService.instance = new _CacheService(context);
    }
  ...
  async initialize() {
    ...
    const stored = this.context.globalState.get(CACHE_KEY, {});
    const now = Date.now();
   ...
   */
  computeHash(content) {
    return crypto2.createHash("sha256").update(content).digest("hex").substring(0, 16);
  }
  /**
   ...
   */
  hasCachedAnalysis(filePath, contentHash) {
    return this.getCachedAnalysis(filePath, contentHash) !== null;
  }
  /**
   * Cache an analysis result
   */
  async cacheAnalysis(filePath, contentHash, result, ttl = DEFAULT_TTL) {
    if (!result.success) {
      return;
    }
    const key = this.getCacheKey(filePath, contentHash);
    ...
    if (keys.length > MAX_CACHE_SIZE) {
      const sortedEntries = keys.map((k) => ({ key: k, cachedAt: this.memoryCache[k].cachedAt })).sort((a, b) => a.cachedAt - b.cachedAt);
...
var vulnerabilityTreeProvider;
var summaryTreeProvider;
var accountTreeProvider;
var inlineCompletionProvider;
var authService;
var historyService;
var cacheService;
var pendingAnalysis = /* @__PURE__ */ new Map();
var inProgressAnalysis = /* @__PURE__ */ new Set();
var lastContentHash = /* @__PURE__ */ new Map();
var DEBOUNCE_DELAY = 1500;
function activate(context) {
  console.log("Python Security Analyzer is now active");
  const config = vscode9.workspace.getConfiguration("pythonSecurityAnalyzer");
  const serverUrl = config.get("serverUrl", "http://127.0.0.1:8000");
  ...
  context.subscriptions.push(vscode9.window.registerUriHandler(uriHandler));
  accountTreeProvider = new AccountTreeProvider(authService);
  accountTreeProvider.setHistoryService(historyService);
  vscode9.window.registerTreeDataProvider("securityAccount", accountTreeProvider);
  vscode9.window.registerTreeDataProvider("securityVulnerabilities", vulnerabilityTreeProvider);
  vscode9.window.registerTreeDataProvider("securitySummary", summaryTreeProvider);
  context.subscriptions.push(
    vscode9.languages.registerHoverProvider("python", new HoverProvider(diagnosticsManager))
  );
  context.subscriptions.push(
  ...
  context.subscriptions.push(
    vscode9.commands.registerCommand("pythonSecurityAnalyzer.analyzeFile", () => analyzeCurrentFile()),
    vscode9.commands.registerCommand("pythonSecurityAnalyzer.analyzeWorkspace", () => analyzeWorkspace()),
    vscode9.commands.registerCommand("pythonSecurityAnalyzer.showPanel", () => showSecurityPanel()),
    vscode9.commands.registerCommand("pythonSecurityAnalyzer.clearDiagnostics", () => clearDiagnostics()),
    vscode9.commands.registerCommand("pythonSecurityAnalyzer.clearCache", () => {
      cacheService.clearCache();
      lastContentHash.clear();
      vscode9.window.showInformationMessage("Security analysis cache cleared");
    }),
    vscode9.commands.registerCommand("pythonSecurityAnalyzer.showFixApplied", (message) => {
      vscode9.window.showInformationMessage(`\u{1F512} ${message}`);
    }),
    // Apply fix for current line via keyboard shortcut (Ctrl+Shift+. or Cmd+Shift+.)
    vscode9.commands.registerCommand("pythonSecurityAnalyzer.applyCurrentLineFix", async () => {
      ...
      if (!fixes || fixes.length === 0) {
        vscode9.window.showInformationMessage("No security issues found in this file");
        return;
      }
      const fix = fixes.find((f) => f.lineNumber === cursorLine1Based);
      if (!fix) {
        vscode9.window.showInformationMessage("No security fix available for this line");
        return;
      }
      ...
      vscode9.window.showInformationMessage(`\u{1F512} ${fix.description}`);
    }),
    vscode9.commands.registerCommand("pythonSecurityAnalyzer.applyFix", async (uri, line, fixedCode) => {
    ...
    vscode9.commands.registerCommand(
      ...
      async (uriString, replacedLine0, endLine0, description) => {
        try {
          const uri = vscode9.Uri.parse(uriString);
          ...
          if (firstToDelete > endLine0 || endLine0 >= doc.lineCount) {
            vscode9.window.showInformationMessage(`\u{1F512} ${description}`);
            return;
          }
          const edit = new vscode9.WorkspaceEdit();
          const deleteRange = new vscode9.Range(
            firstToDelete,
            0,
            endLine0 + 1,
            0
    ...
    // Auth commands
    vscode9.commands.registerCommand("pythonSecurityAnalyzer.login", () => authService.login()),
    vscode9.commands.registerCommand("pythonSecurityAnalyzer.logout", () => authService.logout()),
    vscode9.commands.registerCommand("pythonSecurityAnalyzer.showAuthMenu", () => authService.showAuthMenu()),
    vscode9.commands.registerCommand("pythonSecurityAnalyzer.refreshAccountView", () => accountTreeProvider.refreshStats()),
    vscode9.commands.registerCommand("pythonSecurityAnalyzer.showStats", () => historyService.showStatsPanel()),
    vscode9.commands.registerCommand("pythonSecurityAnalyzer.showHistory", async () => {
      const data = await historyService.getFiles(0, 20);
      if (!data || data.files.length === 0) {
        vscode9.window.showInformationMessage("No analysis history found");
        return;
      }
      ...
      await vscode9.window.showQuickPick(items, {
  ...
  if (config.get("analyzeOnSave", true)) {
    context.subscriptions.push(
      vscode9.workspace.onDidSaveTextDocument((document2) => {
        if (document2.languageId === "python") {
          scheduleAnalysis(document2, true);
        }
      })
    );
  }
  if (config.get("analyzeOnType", false)) {
    context.subscriptions.push(
      vscode9.workspace.onDidChangeTextDocument((event) => {
        if (event.document.languageId === "python") {
          scheduleAnalysis(event.document, false);
        }
      })
    );
  }
  context.subscriptions.push(
    vscode9.workspace.onDidOpenTextDocument((document2) => {
      if (document2.languageId === "python" && document2.uri.scheme === "file") {
        scheduleAnalysis(document2, false);
      }
    })
  );
  let delay = 0;
  vscode9.workspace.textDocuments.forEach((document2) => {
    if (document2.languageId === "python" && document2.uri.scheme === "file") {
      setTimeout(() => scheduleAnalysis(document2, false), delay);
      delay += 500;
    }
  });
  const statusBarItem = vscode9.window.createStatusBarItem(vscode9.StatusBarAlignment.Left, 100);
...
function scheduleAnalysis(document2, immediate = false) {
...
async function analyzeDocument(document2) {
  const config = vscode9.workspace.getConfiguration("pythonSecurityAnalyzer");
  const uri = document2.uri.toString();
  ...
  if (cachedResult) {
    ...
    diagnosticsManager.updateDiagnostics(document2.uri, cachedResult.vulnerabilities);
    vulnerabilityTreeProvider.updateVulnerabilities(document2.uri, cachedResult.vulnerabilities);
    summaryTreeProvider.updateSummary(cachedResult.summary);
    inlineCompletionProvider.updateFixes(document2.uri, cachedResult.vulnerabilities);
    return;
  }
  ...
  console.log("[SecurityAnalyzer] document.fileName:", document2.fileName);
  ...
  try {
    const result = await client.analyzeCode(
      documentText,
      document2.fileName,
      config.get("enableAiExplanations", true),
      config.get("minSeverity", "low")
    );
    if (result.success) {
      lastContentHash.set(uri, contentHash);
      diagnosticsManager.updateDiagnostics(document2.uri, result.vulnerabilities);
      vulnerabilityTreeProvider.updateVulnerabilities(document2.uri, result.vulnerabilities);
      summaryTreeProvider.updateSummary(result.summary);
      inlineCompletionProvider.updateFixes(document2.uri, result.vulnerabilities);
      cacheService.cacheAnalysis(filePath, contentHash, result).catch(
        (err) => console.error("Failed to cache analysis:", err)
      );
      if (historyService && historyService.canSaveHistory()) {
        const shouldSaveCode = config.get("saveCodeSnippetsToHistory", true);
        historyService.saveAnalysis(
          result,
          document2.fileName,
          document2.uri.fsPath,
          shouldSaveCode ? documentText : void 0
      ...
      vscode9.window.showErrorMessage(`Security analysis failed: ${result.error_message}`);
    }
  } catch (error) {
...
async function analyzeCurrentFile() {
  ...
  if (editor.document.languageId !== "python") {
    vscode9.window.showWarningMessage("Current file is not a Python file");
    return;
  }
...
async function analyzeWorkspace() {
  const pythonFiles = await vscode9.workspace.findFiles("**/*.py", "**/node_modules/**");
  if (pythonFiles.length === 0) {
    vscode9.window.showInformationMessage("No Python files found in workspace");
    return;
  }
  ...
  if (!available) {
    vscode9.window.showErrorMessage(
      ...
      if (selection === "Start Server") {
        vscode9.env.openExternal(vscode9.Uri.parse("http://127.0.0.1:8000/docs"));
      }
    });
    return;
  }
  const config = vscode9.workspace.getConfiguration("pythonSecurityAnalyzer");
  const useMultiFileAnalysis = config.get("multiFileAnalysis", true);
  const includeAi = config.get("enableAiExplanations", true);
    ...
    vscode9.window.withProgress(
      ...
      async (progress, token) => {
        ...
        for (const file of pythonFiles) {
          ...
          try {
            const document2 = await vscode9.workspace.openTextDocument(file);
            const relativePath = vscode9.workspace.asRelativePath(file);
            files[relativePath] = document2.getText();
            uriMap[relativePath] = file;
            totalLines += document2.lineCount;
          } catch (err) {
        ...
        if (fileCount === 0) {
          vscode9.window.showWarningMessage("Could not read any Python files");
          return;
        }
        ...
        const vulnerabilitiesByFile = /* @__PURE__ */ new Map();
        ...
        if (result.cross_file_analysis?.vulnerabilities?.length) {
          ...
          for (const cfVuln of result.cross_file_analysis.vulnerabilities) {
            const cfMsg = `\u{1F517} Cross-file ${cfVuln.type}: ${cfVuln.source_file}:${cfVuln.source_line} \u2192 ${cfVuln.sink_file}:${cfVuln.sink_line}`;
            vscode9.window.showWarningMessage(cfMsg);
          }
        }
        ...
        vscode9.window.showInformationMessage(summaryMsg);
      }
    );
  } else {
    vscode9.window.withProgress(
      ...
      async (progress, token) => {
        ...
        for (const file of pythonFiles) {
          ...
          const document2 = await vscode9.workspace.openTextDocument(file);
          await analyzeDocument(document2);
          processed++;
          progress.report({
...
function showSecurityPanel() {
  vscode9.commands.executeCommand("workbench.view.extension.pythonSecurityAnalyzer");
}
function clearDiagnostics() {
  ...
  summaryTreeProvider.clear();
  inlineCompletionProvider.clearAllFixes();
  vscode9.window.showInformationMessage("Security warnings cleared");
}
async function applySecurityFix(uri, line, fixedCode, endLine) {
  ...
  const currentLine = document2.lineAt(startLineIndex);
  const indent = currentLine.text.match(/^(\s*)/)?.[1] || "";
  const formattedFix = fixedCode.split("\n").map(
    (l, i) => i === 0 ? indent + l.trimStart() : indent + l.trimStart()
  ).join("\n");
  const endLineObj = document2.lineAt(endLineIndex);
  const range = new vscode9.Range(startLineIndex, 0, endLineIndex, endLineObj.text.length);
